...
export default function Login() {
  ...
  const [forgotState, forgotAction, forgotPending] = useActionState<
    AdminAuthState,
    FormData
  >(adminForgotPasswordAction, null);

  const isPending = loginPending || forgotPending;
  const state = mode === "login" ? loginState : forgotState;
      ...
      <div className="relative z-10 w-full max-w-[420px]">
        <Card className="border-white/10 bg-[#1c2433]/95 shadow-2xl shadow-black/30">
          ...
          <CardContent className="pt-4">
            <form
              action={mode === "login" ? loginAction : forgotAction}
              className="space-y-4"
            >
              ...
              {state?.success ? (
                <div
                  role="status"
                  className="rounded-lg border border-green-500/30 bg-green-500/10 px-3 py-2 text-sm text-green-300"
                >
                  {state.success}
                </div>
              ) : null}
